...
from utils import create_simple_highres2standard
...
ROI_standard_dir = '../../../data/MRI/{}/anat/ROIs_anat'.format(sub)
if not os.path.exists(ROI_standard_dir):
    os.mkdir(ROI_standard_dir)
...
#     binary_file = os.path.abspath(os.path.join(ROI_standard_dir,'{}.nii.gz'.format(label_name)))
...
preprocessed_functional_dir = os.path.abspath('../../../data/MRI/{}/reg'.format(
        sub))

output_dir = os.path.abspath(os.path.join(anat_dir,'ROI_standard'))
...
for roi in ROI_in_structural:
    ...
    simple_workflow = create_simple_highres2standard(roi = roi,
                                               roi_name = roi_name,
...
masks_standard_to_combine = glob(os.path.join(output_dir,'*pars*.nii.gz'))
for direction in ['lh','rh']:
    parts = [os.path.abspath(item) for item in masks_standard_to_combine if (direction in item)]
    merger = fsl.ImageMaths(in_file = parts[0],
                            in_file2 = parts[1],
                            op_string = '-add')
    merger.inputs.out_file = os.path.abspath(
                            os.path.join(output_dir,
                            'ctx-{}-{}_standard.nii.gz'.format(direction,
                                 'ventrolateralPFC')))
    merger.run()
...
if not os.path.exists(achived):
    os.mkdir(achived)
for part in masks_standard_to_combine:
