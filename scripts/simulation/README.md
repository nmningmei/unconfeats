...
# (and the drop in FCNN performance from 1 to 0.8 with little noise chance). Further investigation 
# shows that this is enssential for the experimetal setting because without the noise example, 
# the trained FCNN model would be biased to predict only one of the classes in median to heavy noise 
# settings, which is something we would want to avoid. 
n_noise                 = 1
```
