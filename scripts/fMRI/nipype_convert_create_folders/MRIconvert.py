...
this script requires mricron-10.2014 while dcm2niix requires mricrogl
"""
...
working_dir = f'/export/home/nmei/nmei/MRI/{sub_name}/*/*/'
working_data = [item for item in glob(os.path.join(working_dir,'*')) if ('Ref' not in item)\
                and ('Localizer' not in item) and ('Phoe' not in item) and ('t1' not in item)]
