...
# interchangable part:
for sub in ['sub-01','sub-02','sub-03','sub-04','sub-05','sub-06','sub-07']:
    for (conscious_state_source,conscious_state_target) in [['unconscious','unconscious'],
                                                            ['conscious','conscious'],
        ...
        for folder_name in ['decoding_LOO_extra','decoding_LOOC_extra']:
            ...
            whole_data_dir          = '../../../../data/BOLD_whole_brain_averaged/{}'.format(sub)
            extra_mask              = '../../../../data/MRI/{}/anat/{}/label/Caudal_Mid_Frontal_bold.nii.gz'.format(sub,sub,)
            roi_name                = 'caudal middle frontal'
            ...
            np.random.seed(12345)
            BOLD_name_source = os.path.join(whole_data_dir,f'whole_brain_{conscious_state_source}.nii.gz')
            masker_source = NiftiMasker(extra_mask,).fit()
            data_source = masker_source.transform(BOLD_name_source)
            
            np.random.seed(12345)
            BOLD_name_target = os.path.join(whole_data_dir,f'whole_brain_{conscious_state_target}.nii.gz')
            masker_target = NiftiMasker(extra_mask,).fit()
            data_target = masker_target.transform(BOLD_name_target)
            
            
            if add_glimpse and conscious_state_source == 'conscious':
                _data_source        = masker_source.transform(BOLD_name_source.replace(conscious_state_source,'glimpse'))
                data_source         = np.concatenate([data_source,_data_source])
            if add_glimpse and conscious_state_target == 'conscious':
                _data_target        = masker_source.transform(BOLD_name_source.replace(conscious_state_source,'glimpse'))
