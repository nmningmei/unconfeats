...
meta_template = '3.1.add_noise_to_images_record_trials_for_meta.py'
scripts_folder = 'metabatch'
...
for ii,row in df.iterrows():
    ...
    for template in [meta_template]:
        ...
        with open(new_scripts_name,'w') as new_file:
            ...
            old_file.close()
        new_file.close()
    new_batch_script_name = os.path.join(scripts_folder,f'META{ii+1}')
    content = f"""#!/bin/bash
#PBS -q bcbl
...
python "{scripts[0].split('/')[-1]}"
    ...
    with open(new_batch_script_name,'w') as f:
        f.write(content)
        f.close()
    collections.append(f"qsub META{ii+1}")
