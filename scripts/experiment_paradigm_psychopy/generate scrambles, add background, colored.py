...
processed_dir = '../experiment_stimuli/experiment_images_tilted_scramble_colored_2'
if not os.path.exists(processed_dir):
    os.mkdir(processed_dir)
saving_dir = '../experiment_stimuli/cor_bc_bl'
...
for category,corrAn in categories.items():
    for subcategory in glob(os.path.join(figure_dir,category,'*')):
        ...
        for image_path in tqdm(sampled):
            ...
            im_tran = trans(img,250,'average');im_tran
            img_cropped = white_to_transparency(im_tran)
                
            img_cropped_bw = img_cropped.convert('RGB')
            temp = []
            for ii in range(5):
                img_scram = Image.fromarray(scramble(img_cropped)).convert('RGB');img_scram
            ...
            img_scram_background = np.array(Image.fromarray(scramble(img_cropped)).convert('RGB'))
